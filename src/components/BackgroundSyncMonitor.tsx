...
export default function BackgroundSyncMonitor({ userId, accountId, onSyncComplete }: BackgroundSyncMonitorProps) {
  ...
  if (activeSyncs.length === 0 && !isInitialImportStarted) {
    ...
    return (
      <div className="flex items-center gap-2 text-sm text-muted-foreground p-4 border rounded-lg bg-muted/30">
        ...
        <span>Ready to sync your emails</span>
        <Button 
          size="sm" 
          onClick={startInitialImport}
          disabled={isInitialImportStarted}
        >
          Start Import
        </Button>
      </div>
    ...
    <div className="space-y-2">
      {activeSyncs.map((sync) => {
        ...
        return (
          <div 
            key={sync.id}
            className="flex items-center gap-2 text-sm text-muted-foreground p-3 border rounded-lg bg-muted/30"
          >
